	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  if ((prefetch_type < 0)|(prefetch_type >1))
    fatal("prefetcher type `%d'must be 0 or 1", prefetch_type);
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  stat_reg_counter(sdb, buf, "total number of prefetches", &cp->prefetches, 0, NULL);   //Print the number of prefetches
...
{
   if (cache_probe(cp, addr + cp->bsize))      //check whether the next block contaied in the cache
...
void generate_prefetch(struct cache_t *cp, md_addr_t addr) 
{
	switch(cp->prefetch_type) 
        {
		case 0:          // prefetching is not enabled;              
		   break;         // do nothing
		case 1:           //Enable prefetch
		   next_line_prefetcher(cp, addr);  //Call prefetch function, use nextline prefetch algorithm
		   break;
	}
	     ...
	     int prefetch)		/* 1 if the access is a prefetch, 0 if it is not */
{
  ...
  if (prefetch == 0) /* If the instruction is not a prefetch instruction, generate a prefetch */
  {	
